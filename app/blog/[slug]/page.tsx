      ...
      elements.push(
        <div key={i} className="overflow-x-auto my-6">
          <table className="w-full text-sm border-collapse">
            <thead>
              <tr className="border-b border-[#9A7B2F]/30">
                {headers.map((h, hi) => (
                  <th key={hi} className="text-left py-3 px-4 text-[#9A7B2F] font-semibold">
                    {h}
                  </th>
                ))}
              </tr>
        ...
        <ul key={i} className="space-y-2 my-4">
          {checkItems.map((item, ii) => (
            <li key={ii} className="flex items-start gap-3 text-sm text-[#a8a8a8]">
              <span className="w-4 h-4 rounded border border-[#9A7B2F]/30 shrink-0 mt-0.5" />
              {item}
            </li>
          ))}
        </ul>
      ...
      <section className="pt-32 pb-12 bg-[#050505]">
        <div className="max-w-4xl mx-auto px-6 lg:px-8">
          ...
          <nav className="flex items-center gap-2 text-xs text-[#404040] mb-8">
            ...
            <span>/</span>
            <span className="text-[#9A7B2F] line-clamp-1">{post.title}</span>
          </nav>

          <div className="flex items-center gap-3 mb-6">
            <span className="text-xs font-medium text-[#9A7B2F] border border-[#9A7B2F]/30 px-3 py-1 rounded-full">
              {post.category}
            </span>
      ...
      <section className="py-12 bg-[#0a0a0a]">
        <div className="max-w-4xl mx-auto px-6 lg:px-8">
          ...
          <div className="card-premium p-6 flex items-start gap-4">
            <div className="w-12 h-12 rounded-full bg-gradient-to-br from-[#B59A55] to-[#796020] flex items-center justify-center text-black font-bold text-lg shrink-0">
              K
            </div>
            <div>
              ...
              <Link href="/sobre-mi" className="text-xs text-[#9A7B2F] mt-2 inline-block hover:text-[#B59A55]">
                Conocer más →
              </Link>
            </div>
          </div>
          ...
          <div className="max-w-4xl mx-auto px-6 lg:px-8">
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {relatedPosts.map((rpost) => (
                ...
                >
                  <div className="text-xs text-[#9A7B2F] mb-2">{rpost.category}</div>
                  <h3 className="text-sm font-semibold text-white mb-2 leading-snug group-hover:text-[#B59A55] transition-colors line-clamp-2">
                    {rpost.title}
                  </h3>
