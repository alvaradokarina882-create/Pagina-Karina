      ...
      elements.push(
        <div key={i} className="overflow-x-auto my-6">
          <table className="w-full text-sm border-collapse">
            <thead>
              <tr className="border-b border-[#A98224]/30">
                {headers.map((h, hi) => (
                  <th key={hi} className="text-left py-3 px-4 text-[#A98224] font-semibold">
                    {h}
                  </th>
                ))}
              </tr>
        ...
        <ul key={i} className="space-y-2 my-4">
          {checkItems.map((item, ii) => (
            <li key={ii} className="flex items-start gap-3 text-sm text-[#a8a8a8]">
              <span className="w-4 h-4 rounded border border-[#A98224]/30 shrink-0 mt-0.5" />
              {item}
            </li>
          ))}
        </ul>
      ...
      <section className="pt-32 pb-12 bg-[#050505]">
        <div className="max-w-4xl mx-auto px-6 lg:px-8">
          ...
          <nav className="flex items-center gap-2 text-xs text-[#404040] mb-8">
            ...
            <span>/</span>
            <span className="text-[#A98224] line-clamp-1">{post.title}</span>
          </nav>

          <div className="flex items-center gap-3 mb-6">
            <span className="text-xs font-medium text-[#A98224] border border-[#A98224]/30 px-3 py-1 rounded-full">
              {post.category}
            </span>
      ...
      <section className="py-12 bg-[#0a0a0a]">
        <div className="max-w-4xl mx-auto px-6 lg:px-8">
          ...
          <div className="card-premium p-6 flex items-start gap-4">
            <div className="w-12 h-12 rounded-full bg-gradient-to-br from-[#C3A04A] to-[#8C6918] flex items-center justify-center text-black font-bold text-lg shrink-0">
              K
            </div>
            <div>
              <div className="font-semibold text-white mb-1">Karina Alvarado</div>
              <p className="text-xs text-[#606060] leading-relaxed">
                Fundadora y CEO de Alvarado's Beauty & Barber, autora y conferencista
              ...
              <Link href="/sobre-mi" className="text-xs text-[#A98224] mt-2 inline-block hover:text-[#C3A04A]">
                Conocer más →
              </Link>
            </div>
          </div>
          ...
          <div className="max-w-4xl mx-auto px-6 lg:px-8">
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {relatedPosts.map((rpost) => (
                ...
                >
                  <div className="text-xs text-[#A98224] mb-2">{rpost.category}</div>
                  <h3 className="text-sm font-semibold text-white mb-2 leading-snug group-hover:text-[#C3A04A] transition-colors line-clamp-2">
                    {rpost.title}
                  </h3>
