  ...
  return (
    <>
      ...
      <section className="min-h-screen flex items-center justify-center bg-[#0a0a0a] relative overflow-hidden">
        ...
        <div className="absolute top-1/4 right-0 w-[600px] h-[600px] rounded-full bg-[#B59A55]/4 blur-[120px]" />

        <div className="relative z-10 max-w-7xl mx-auto px-6 lg:px-8 pt-24 pb-16 text-center">
          <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full border border-[#9A7B2F]/30 bg-[#9A7B2F]/5 mb-8">
                ...
                <div key={t.name} className="card-premium p-6">
                  <div className="flex gap-1 mb-4">
                    {Array.from({ length: t.stars }).map((_, i) => (
                      <span key={i} className="text-[#B59A55] text-sm">★</span>
                    ))}
                  </div>
                  <p className="text-[#a8a8a8] text-sm leading-relaxed mb-4 italic">
                    &ldquo;{t.text}&rdquo;
                  </p>
                  <div className="text-xs font-semibold text-[#9A7B2F]">{t.result}</div>
                </div>
              ))}
            </div>
          </div>
